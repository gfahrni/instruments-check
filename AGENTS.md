...
**2 colonnes swipeables** (Edwin → 🎹 Piano / Ambre → 🎻 Violon), chaque colonne défile verticalement
(`scroll-snap`) entre **2 panneaux** : panneau 1 = mission/étoile, panneau 2 = cadeau/carte.
Bouton **Fait** → étoile débloquée **immédiatement**. Bouton **🎁 Cadeau Bonus** (popup de
confirmation) → débloque le panneau 2 (TP automatique dessus), qui affiche une **carte du jour**
du set de l'enfant (image seule). Bouton **↺ Recommencer** par enfant.
...
## Structure
- `index.html` : layout 2 colonnes swipeables + logique (JS simple, tout inline).
- `instruments-v2.css` : thème versionné. **Renommer à chaque changement visuel**
  (ex: `instruments-v3.css`) pour forcer le cache Safari.
...
- **Reset quotidien** : `resetIfNewDay()` / `clearAll()` (`index.html`) efface tout (mission, étoile,
  cadeau, carte) à chaque nouveau jour (clé `instruments-v1-day`). Recharge aussi au retour d'app (visibilitychange/pageshow).
- **Carte du jour** : sélection **stable toute la journée** (`hash(date|enfant) % nbCartes`), mémorisée
  sous `instruments-v1-carte-<enfant>` ; change uniquement le lendemain.
- **Bouton ↺ Recommencer** (`clearKid`) : efface mission + cadeau de l'enfant, **sans** toucher à la
  sélection de la carte du jour. Remonte la colonne sur le panneau 1.
- État sous préfixe `instruments-v1-` (`fait-<enfant>`, `bonus-<enfant>`, `carte-<enfant>`, `day`).
